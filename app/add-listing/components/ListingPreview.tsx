...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="card-base rounded-2xl p-5 shadow-sm">
      {title && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl px-5 py-3">
        ...
        <button onClick={onEdit} className="text-xs font-semibold text-amber-700 dark:text-amber-300 hover:underline ml-4 shrink-0">
          ← Edit
        </button>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Section title="">
            <div className="flex items-start justify-between gap-3">
              ...
              {form.offerType && (
                <span className="shrink-0 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full">
                  {form.offerType}
                </span>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {activeFeatures.map((feat) => (
                  <div key={feat} className="flex gap-2 items-center text-sm text-gray-700 dark:text-[#c4b8e8]">
                    <FaCheck className="text-xs p-1 w-5 h-5 rounded-full bg-blue-100 dark:bg-[#2d1e5f] text-blue-600 dark:text-[#58a6ff] shrink-0" />
                    {feat}
                  </div>
                ))}
              </div>
      ...
      {/* ── Sticky submit bar ────────────────────────────────────────────────── */}
      <div className="sticky bottom-0 card-base border-t px-4 py-4 -mx-4 flex items-center justify-between gap-4 z-10">
        <button
          onClick={onEdit}
          disabled={isPublishing}
          className="btn-outline px-6 py-2.5 rounded-full text-sm font-semibold disabled:opacity-40"
        >
          ← Back to Edit
        </button>
