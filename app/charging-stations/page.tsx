...
export default function ChargingStationsPage() {
  ...
  return (
    <div className="bg-white dark:bg-[#0a0f2e] min-h-screen flex flex-col">
      <Navbar />

      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <div className="card-base border-b px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
        <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={fetchStations} className="p-2 rounded-lg hover:bg-primary-light transition" title="Refresh">
            <RefreshCw className={`w-4 h-4 text-muted ${loading ? "animate-spin" : ""}`} />
          </button>
          <button onClick={() => setShowSidebar(true)}
            className="lg:hidden flex items-center gap-1.5 px-3 py-2 btn-outline text-xs font-semibold">
            <SlidersHorizontal className="w-3.5 h-3.5" /> Filters
          </button>
          <button onClick={() => setShowForm(true)}
            className="flex items-center gap-1.5 px-3 py-2 btn-primary text-xs font-semibold">
            <Plus className="w-3.5 h-3.5" /> Add Station
          </button>
          <button onClick={() => setShowAbout(true)}
            className="hidden md:block text-xs text-primary hover:underline font-medium">
            About EV Charging
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden" style={{ height: "calc(100vh - 120px)" }}>
        ...
        {/* Desktop sidebar */}
        <aside className="hidden lg:block w-64 shrink-0 border-r card-base overflow-y-auto">
          <FilterPanel
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute top-3 left-3 z-10 card-base rounded-xl shadow-lg px-3 py-2">
            <p className="text-xs font-semibold text-foreground">{filtered.length} stations</p>
          </div>

          {/* Legend */}
          <div className="absolute bottom-6 left-3 z-10 card-base rounded-xl shadow-lg p-3 text-[10px] space-y-1">
            {[
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-[#18122b] max-w-2xl w-full rounded-2xl shadow-2xl p-6 relative border border-gray-100 dark:border-[#2d1e5f]">
            ...
            <div className="mt-5 p-4 bg-[#7b2ff2]/10 rounded-xl border border-[#7b2ff2]/20 flex items-center gap-3">
              <IoPhonePortrait className="text-2xl text-[#7b2ff2] shrink-0" />
              <div>
