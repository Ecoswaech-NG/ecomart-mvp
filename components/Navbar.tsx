...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className="hidden md:flex justify-between items-center px-6 py-3 sticky top-0 z-20 backdrop-blur-md border-b"
        style={{ background: `linear-gradient(135deg, var(--color-background) 0%, color-mix(in srgb, var(--color-card) 60%, var(--color-background) 40%) 100%)` }}
      >
        ...
        <nav className={`flex items-center justify-between card-base rounded-full shadow-lg px-1.5 py-1.5 mx-4 flex-1 max-w-5xl backdrop-blur-md`}>
          ...
          <div className="relative flex-1 flex justify-center" ref={shopDropdownRef}>
            ...
            {shopDropdownOpen && (
              <div className="absolute top-full left-0 mt-3 w-48 card-elevated shadow-lg rounded-xl z-50 overflow-hidden">
                <Link href="/chargers"    className="flex px-5 py-3 text-xs hover:bg-primary-light text-foreground items-center gap-2" onClick={() => setShopDropdownOpen(false)}><LightningIcon /> Chargers</Link>
                <Link href="/accessories" className="flex px-5 py-3 text-xs hover:bg-primary-light text-foreground items-center gap-2" onClick={() => setShopDropdownOpen(false)}><ToolsIcon /> Accessories</Link>
              </div>
            )}
          </div>
          ...
          <Link href={userLoggedIn ? "/add-listing" : "/login"} className="flex-1 px-1">
            <span className={`btn-primary block w-full text-center rounded-full text-[11px] whitespace-nowrap ${
                isActive("/add-listing") ? "opacity-90" : ""
            }`}>
              SELL HERE
            </span>
          </Link>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button onClick={toggle}
            className="btn-outline rounded-full w-9 h-9 flex items-center justify-center transition-all"
          >
          ...
          {userLoggedIn ? (
            <div className="relative">
              <button className="dropdown-toggle" onClick={() => setDropdownOpen((o) => !o)}>
                <span className="text-primary"><UserCircleIcon /></span>
              </button>
              {dropdownOpen && (
                <div ref={dropdownRef} className="absolute top-full right-0 mt-2 w-56 card-elevated shadow-lg rounded-xl z-50">
                  <div className="px-5 py-3 border-b border-current border-opacity-10">
                    <p className="text-xs text-foreground font-semibold truncate">{user?.fullName}</p>
                  ...
                  {[
                    ...
                    <Link key={label} href={href} onClick={() => setDropdownOpen(false)}
                      className="flex px-5 py-3 text-sm hover:bg-primary-light text-foreground items-center gap-2">
                      {icon} {label}
                    </Link>
                  ))}
                  <button onClick={() => { setDropdownOpen(false); handleLogout(); }}
                    className="w-full text-left px-5 py-3 text-sm hover:bg-primary-light text-red-500 flex items-center gap-2 rounded-b-xl transition">
                    <LogoutIcon /> Logout
                  </button>
                </div>
              )}
            ...
            <Link href="/login">
              <span className="btn-primary rounded-full px-5 py-2 text-[12px] font-bold whitespace-nowrap">
                Sign In / Register
              </span>
            </Link>
          )}
      ...
      {/* MOBILE SECTION */}
      <div className="flex justify-between items-center p-3 sticky top-0 z-30 md:hidden shadow-xl backdrop-blur-md card-base border-b"
      >
        <Link href="/home">
          <Image src="/logo.svg" alt="Ecomart" width={40} height={40} className="w-10 h-10" />
        </Link>
        <div className="flex items-center gap-2">
          <button onClick={toggle} className="btn-outline rounded-full w-8 h-8 flex items-center justify-center"
          >
            {isDark ? <SunIcon /> : <MoonIcon />}
          </button>
          <button onClick={() => setMobileMenuOpen((o) => !o)} className="p-2 text-foreground">
            <MenuIcon />
          </button>
        </div>
        {mobileMenuOpen && (
          <div className="absolute top-full left-0 right-0 card-base shadow-lg border-t z-40">
            {[
              ...
              <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)}
                className={`flex px-6 py-4 items-center gap-3 text-sm border-b border-current border-opacity-10 ${isActive(href) ? "bg-primary-light text-primary font-semibold" : "text-foreground"}`}>
                {icon} {label}
              </Link>
            ))}
            <div className="border-t border-current border-opacity-10">
              {userLoggedIn ? (
                <button onClick={() => { setMobileMenuOpen(false); handleLogout(); }}
                  className="w-full text-left px-6 py-4 text-sm hover:bg-primary-light text-red-500 flex items-center gap-3">
                  <LogoutIcon /> Logout
                </button>
              ) : (
                <Link href="/login" onClick={() => setMobileMenuOpen(false)}
                  className="flex px-6 py-4 text-foreground hover:bg-primary-light items-center gap-3 text-sm font-semibold">
                  Sign In / Register
                </Link>
              )}
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex md:hidden card-base rounded-t-2xl shadow-2xl justify-between items-center px-6 border-t"
        style={{ minHeight: 70, paddingTop: 16, paddingBottom: "calc(16px + env(safe-area-inset-bottom))" }}>
        <Link href="/home" className={`flex flex-col items-center flex-1 ${isActive("/home") ? "text-primary" : "text-muted"}`}>
          <HomeIcon /><span className="text-[10px] mt-1">Home</span>
        </Link>
        <Link href="/search" className={`flex flex-col items-center flex-1 ${isActive("/search") ? "text-primary" : "text-muted"}`}>
        ...
        <div className="relative flex flex-col items-center flex-1 z-10 cursor-pointer"
          ...
          <div className={`w-12 h-12 flex items-center justify-center rounded-xl rotate-45 -mt-8 shadow-lg border-2 border-white transition-all ${isActive("/add-listing") ? "bg-primary text-foreground" : "bg-secondary text-white"}`}>
            <div className="-rotate-45"><SellIcon /></div>
          </div>
          <span className="text-[10px] mt-2 font-semibold text-foreground">{userLoggedIn ? "SELL" : "SIGN IN"}</span>
        </div>
        <Link href="/messages" className={`flex flex-col items-center flex-1 ${isActive("/messages") ? "text-primary" : "text-muted"}`}>
          <MessageIcon /><span className="text-[10px] mt-1">Messages</span>
        </Link>
        <Link href="/user/dashboard" className={`flex flex-col items-center flex-1 ${isActive("/user/dashboard") ? "text-primary" : "text-muted"}`}>
