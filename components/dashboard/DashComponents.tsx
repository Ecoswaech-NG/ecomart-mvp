  ...
  return (
    <div>
      <h2 className="font-bold text-2xl text-foreground mb-6">
        Saved Vehicles <span className="text-muted text-lg font-normal">({ids.length})</span>
      </h2>

      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {Array.from({length:3}).map((_,i)=>(
            <div key={i} className="h-56 rounded-2xl card-base animate-pulse"/>
          ))}
        </div>
      ) : listings.length === 0 ? (
        <div className="text-center py-20">
          <Bookmark className="w-12 h-12 text-gray-300 mx-auto mb-4" />
          <p className="font-semibold text-foreground">No saved vehicles</p>
          <p className="text-sm text-muted mt-1">Bookmark listings from the search page</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {listings.map((l) => (
            <div key={l.id} className="card-elevated rounded-2xl overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="flex gap-2 mt-3">
                  <Link href={`/listing-details/${l.id}`} className="flex-1">
                    <button className="btn-primary w-full py-1.5 text-xs">View</button>
                  </Link>
                  <button onClick={() => remove(l.id)}
                    className="btn-outline px-3 py-1.5 text-xs text-red-500 border-red-500/50">
                    Remove
                  </button>
                </div>
              </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-bold text-2xl text-foreground">My Rental Listings</h2>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {rentals.map((rental) => (
            <div key={rental.id} className="card-interactive rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition">
              <div className="relative h-40 bg-linear-to-br from-blue-400 to-blue-600 flex items-center justify-center">
          ...
          <form onSubmit={handleSubmit} className="bg-white dark:bg-[#18122b] rounded-2xl shadow-2xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea name="rules" className={`${inputCls} min-h-20 resize-none`} value={form.rules} onChange={handleChange} placeholder="One rule per line" />
              </div>
            </div>
...
export function MyLogistics() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-bold text-2xl text-foreground">My Logistics Services</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        {CATS.map(({ icon: Icon, label, color }) => (
          <div key={label} className="flex flex-col items-center card-base rounded-2xl p-5 shadow-sm">
            <Icon className={`text-3xl mb-2 ${color}`} />
            <p className="font-semibold text-xs text-center text-foreground">{label}</p>
            <p className="text-[10px] text-muted mt-1">0 active</p>
          </div>
        ))}
      </div>

      <div className="flex flex-col items-center justify-center py-12">
        <FaTruck className="text-5xl text-gray-200 mb-4" />
        <p className="font-semibold text-muted">No services yet</p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleSubmit} className="card-base rounded-2xl shadow-2xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
            <button type="button" onClick={() => setShowModal(false)} className="absolute top-4 right-4 text-muted hover:text-foreground text-xl">×</button>
            <h3 className="font-bold text-lg text-foreground mb-5">Create Logistics Service</h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea name="coverage" className={`${inputCls} min-h-20 resize-none`} value={form.coverage} onChange={handleChange} placeholder="Lagos, Abuja..." />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <Button variant="outline" type="button" className="flex-1 rounded-full" onClick={() => setShowModal(false)}>Cancel</Button>
